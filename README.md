...
Al no usar un framework como React o Angular, no hay un comando para correr un servidor local que transpila el código TS a JS de forma automática. Hay que ejecutar el comando `npx tsc` para transpilarlo de forma manual. En el `tsconfig.json` dice que el código TS debe ir en la carpeta `/src` y el código resultante irá en la carpeta `/dist`
